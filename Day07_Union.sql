...
select * from kadro where sirket = 'Honda'
union
select * from kadro where sirket = 'Tofas';


-- 2) Honda, Ford ve Tofas'ta calisan benzersiz isimdeki
-- personelleri listeleyin
select  isim from kadro where sirket = 'Honda'
union
select  isim from kadro where sirket = 'Tofas'
union
select  isim from kadro where sirket = 'Ford';
...
select  isim, maas, sehir from kadro where sirket = 'Honda'
union all
select  isim, maas, sehir from kadro where sirket = 'Tofas'
union all
select  isim, maas, sehir from kadro where sirket = 'Ford';
-- 5) Maasi 4000'den fazla olan isci isimlerini ve
-- 5000'den fazla
-- maas alinan sehirleri gosteren sorguyu yaziniz

select  isim from kadro where maas > 4000
union all
select  sehir from kadro where maas > 5000;


select  maas as MehmetOzturk_IstanbulMaaslari from kadro where isim = 'Mehmet Ozturk'
union all
select  maas from kadro where sehir='Istanbul' ;
...
select maas as maas_liste from kadro where isim='Mehmet  Ozturk'
union
select maas from kadro where sehir = 'Istanbul'
order by maas_liste desc;
...
select sehir as sehir_isim from kadro where maas > 3000
union
select isim from kadro where maas < 5000;

-- 9) 5000'den az maas alanlarin, arti Honda calisani olmayanlarin bilgilerini
-- listeleyen bir sorgu yaziniz.

select * from kadro where maas < 5000
union
select * from kadro where sirket != 'Honda';

-- <> ile != ayni anlami tasimaktadir!!!!!!!

-- 10) Ismi Mehmet Ozturk olanlari ve ek olarak Istanbul'da calismayanlarin isimlerini ve
-- sehirlerini listeleyen sorguyu yaziniz.

select isim,sehir from kadro where isim ='Mehmet Ozturk'
union
select isim,sehir from kadro where sehir <> 'Istanbul';
